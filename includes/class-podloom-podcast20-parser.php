...
class Podloom_Podcast20_Parser {
	 ...
	 */
	const PODCAST_NAMESPACES = array(
	...
	public function parse_from_simplepie_channel( $feed ) {
		...
		return array(
			'funding'     => $funding,
			'transcripts' => array(), // Channel-level transcripts are rare
	...
	private function parse_transcript_tag( $item, $ns ) {
		...
		foreach ( $transcript_elements as $transcript ) {
			...
			$transcripts[] = array(
				'url'      => esc_url_raw( $url ),
				'type'     => sanitize_text_field( $type ? $type : 'text/plain' ),
				'language' => sanitize_text_field( $language ? $language : 'en' ),
				'rel'      => sanitize_text_field( $rel ? $rel : 'captions' ),
	...
	private function parse_person_tag( $item, $ns ) {
		...
		foreach ( $person_elements as $person ) {
			...
			$people[] = array(
				'name'  => sanitize_text_field( $name ),
				'role'  => sanitize_text_field( $role ? $role : 'guest' ),
				'group' => sanitize_text_field( $group ),
				'img'   => ! empty( $img ) ? esc_url_raw( $img ) : null,
				'href'  => ! empty( $href ) ? esc_url_raw( $href ) : null,
			);
	...
	public function get_empty_structure() {
		return array(
			...
			'transcripts'    => array(),
			'people'         => array(),
			'people_channel' => array(), // Hosts from channel level
			'people_episode' => array(), // Guests from episode level
			'chapters'       => null,
		);
	}
	...
	public function merge_data( $item_data, $channel_data ) {
		$channel_people = $channel_data['people'] ?? array();
		$episode_people = $item_data['people'] ?? array();
		...
		return array(
			...
			'transcripts'    => array_merge(
				$item_data['transcripts'] ?? array(),
				$channel_data['transcripts'] ?? array()
			),
