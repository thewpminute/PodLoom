...
class Podloom_API {
	...
	public function __construct( $api_key = null ) {
		$this->api_key = $api_key ? $api_key : get_option( 'podloom_api_key', '' );
	}
	 ...
	 */
	private function cached_request( $endpoint, $method = 'GET', $params = array() ) {
		// Check if caching is enabled
...
function podloom_ajax_get_shows() {
	...
	if ( is_wp_error( $shows ) ) {
		wp_send_json_error( array( 'message' => $shows->get_error_message() ) );
	}
...
function podloom_ajax_get_episodes() {
	...
	if ( is_wp_error( $episodes ) ) {
		wp_send_json_error( array( 'message' => $episodes->get_error_message() ) );
	}
...
function podloom_ajax_get_episode() {
	...
	if ( empty( $episode_id ) ) {
		wp_send_json_error( array( 'message' => 'Episode ID is required' ) );
	}
	...
	if ( is_wp_error( $episode ) ) {
		wp_send_json_error( array( 'message' => $episode->get_error_message() ) );
	}
...
function podloom_clear_all_cache() {
	...
	if ( wp_using_ext_object_cache() ) {
		// Try group-based flush first (supported by Redis Object Cache, etc.)
		if ( function_exists( 'wp_cache_flush_group' ) ) {
			wp_cache_flush_group( 'podloom' );
			wp_cache_flush_group( 'podloom_editor' );
		} else {
			// Fallback: delete known cache keys individually
			// Note: We avoid wp_cache_flush() as it clears ALL cached data site-wide
			wp_cache_delete( 'rss_typography_cache', 'podloom' );
		}
	}
