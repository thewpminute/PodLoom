...
define( 'PODLOOM_PLUGIN_VERSION', '2.17.1' );
...
function podloom_fetch_transcript() {
	...
	if ( ! in_array( strtolower( (string) $parsed_scheme ), array( 'http', 'https' ), true ) ) {
		wp_send_json_error( array( 'message' => 'Invalid URL scheme' ), 400 );
	}

	// Second SSRF layer: WordPress built-in URL validator (filterable for advanced users).
	$http_validated = apply_filters( 'podloom_transcript_http_validate_url', wp_http_validate_url( $url ), $url );
	if ( ! $http_validated ) {
		wp_send_json_error( array( 'message' => 'URL failed safety validation' ), 403 );
	}
	...
	// Size limit used both as a pre-download cap and a post-download check.
	$max_size = apply_filters( 'podloom_transcript_max_size', 2 * 1024 * 1024 ); // 2MB default

	// Fetch the transcript using WordPress HTTP API.
	// 'reject_unsafe_urls' handles SSRF protection (blocks private IPs, localhost, etc.)
	// 'limit_response_size' aborts the download before it hits memory when the server
	// streams more than $max_size bytes.
	...
	// Body is read only after all header checks pass.
